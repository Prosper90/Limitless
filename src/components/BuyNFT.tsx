...
export const BuyNFT: React.FC = () => {
  ...
  const { nftPrice, mintNFT, isPending: isMinting, isConfirming: isConfirmingMint, isSuccess: mintSuccess, error: mintError } = useLimitlessNFT();
  const { balance, allowance, symbol, approve, isPending: isApproving, isConfirming: isConfirmingApprove, isSuccess: approveSuccess, refetchAllowance } = useStablecoin();
    ...
    <div className="container py-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-10">
          ...
          <p className="text-gray-400">Purchase an NFT to start earning 1 token per day forever</p>
        </div>
          ...
          <div className="lg:col-span-2">
            <div className="nerko-card sticky top-28">
              ...
              <p className="text-gray-400 text-sm mb-4">
                Each NFT entitles you to receive 1 LIMITLESS token per day, forever. No staking required.
              </p>
              <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl">
                <div>
                  <p className="text-gray-400 text-sm">Price</p>
                  <p className="text-3xl font-bold gradient-text">${nftPrice}</p>
                </div>
                <div className="text-right">
                  <p className="text-gray-400 text-sm">Your Balance</p>
                  <p className="text-xl font-bold">{parseFloat(balance).toFixed(2)} {symbol}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="nerko-card">
              ...
              <p className="text-gray-500 text-xs mt-3">
                If someone referred you, enter their wallet address to give them commission
              </p>
            </div>
            ...
            <div className="nerko-card">
              {!hasEnoughBalance ? (
                <div className="text-center py-4">
                  ...
                  <p className="text-red-400 font-semibold mb-2">Insufficient {symbol} Balance</p>
                  <p className="text-gray-400 text-sm">You need at least ${nftPrice} {symbol} to purchase an NFT</p>
                </div>
              ) : (
                <>
                  {/* Progress Steps */}
                  <div className="flex items-center gap-4 mb-6">
                    <div className={`flex items-center gap-2 ${step === "approve" ? "text-purple-400" : "text-green-400"}`}>
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step === "approve" ? "bg-purple-600" : "bg-green-600"}`}>
                        {step === "mint" ? (
                    ...
                    <div className="flex-1 h-1 bg-white/10 rounded">
                      <div className={`h-full rounded transition-all ${step === "mint" ? "bg-green-600 w-full" : "bg-purple-600 w-0"}`} />
                    </div>
                    <div className={`flex items-center gap-2 ${step === "mint" ? "text-purple-400" : "text-gray-500"}`}>
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step === "mint" ? "bg-purple-600" : "bg-white/10"}`}>
                        <span>2</span>
                      </div>
              ...
              {mintSuccess && (
                <div className="mt-4 p-4 bg-green-600/20 border border-green-600/30 rounded-xl text-center">
                  ...
                  <p className="text-green-400 font-bold text-lg">NFT Purchased Successfully!</p>
                  <p className="text-gray-400 text-sm mt-1">You will now earn 1 LIMITLESS token per day</p>
                </div>
              )}
              ...
              {mintError && (
                <div className="mt-4 p-4 bg-red-600/20 border border-red-600/30 rounded-xl text-center">
                  ...
                  <p className="text-gray-400 text-sm mt-1">{mintError.message}</p>
                </div>
              )}
            </div>
            ...
            {address && (
              <div className="nerko-card">
                ...
                <p className="text-gray-500 text-xs mt-3">
                  Share this link to earn up to $25 commission per NFT sale (6 levels)
                </p>
              </div>
            )}
          </div>
